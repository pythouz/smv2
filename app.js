...
الهوية Nostr
========================================================= */
// دالة مساعدة لتحويل hex إلى Uint8Array (مطلوب في nostr-tools v1)
...
async function initIdentity() {
  try {
    ...
    if (!isValidHex) {
      ...
      hexSk = Array.from(generated).map(byte => byte.toString(16).padStart(2, '0')).join('');
      localStorage.setItem(storageKey, hexSk);
      showToast('تم إنشاء هوية جديدة. صدّرها واحفظها!', 'info');
    }
    
    secretKeyHex = hexSk;
    pk = NostrTools.getPublicKey(hexToBytes(secretKeyHex));
    npub = NostrTools.nip19.npubEncode(pk);
    usingNip07 = false;
    updateIdentityUI();
...
async function signEvent(eventTemplate) {
  ...
  return NostrTools.finalizeEvent(eventTemplate, hexToBytes(secretKeyHex));
}
...
function exportKey() {
  ...
  try {
    const nsec = NostrTools.nip19.nsecEncode(hexToBytes(secretKeyHex));
...
function renderRoomDirectory() {
  ...
  container.innerHTML = rooms.map(room => `
    <button onclick="joinDiscoveredRoom('${room.name.replace(/'/g, "\\'")}')" class="w-full flex items-center justify-between gap-3 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition rounded-xl px-4 py-3 text-right">
...
function setupVAD() {
  ...
  try {
    ...
    const interval = setInterval(() => {
      ...
      if (status) {
        status.textContent = volume > 12 ? 'الحالة: تتحدث الآن 🎙️' : 'الحالة: متصل (صامت)';
      }
    }, 200);
  } catch (e) {}
}
